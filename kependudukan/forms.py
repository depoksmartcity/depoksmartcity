...
from kependudukan.models import TIMESLOT_LIST, KECAMATAN_CHOICES, KELURAHAN_CHOICES, PERMOHONAN_CHOICES, RequestKTP
...
# creating a form 
class RequestKTPForm(forms.ModelForm):
    class Meta:
        model = RequestKTP
        fields = ['kecamatan', 'kelurahan', 'permohonan', 'nama_lengkap', 'nomor_kk', 'nik', 'alamat', 'rt', 'rw', 'kode_pos', 'nomor_hp', 'schedule_date', 'schedule_time']
        widgets = {
            'kecamatan' : forms.Select(attrs={'class': 'form-control', 'name': 'kecamatan'}),
            'kelurahan' : forms.Select(attrs={'class': 'form-control', 'name': 'kelurahan'}),
            'permohonan' : forms.Select(attrs={'class': 'form-control', 'name': 'permohonan'}),

            'nama_lengkap' : forms.TextInput(attrs={'class': 'form-control', 'name': 'nama_lengkap'}),
            'nomor_kk' : forms.TextInput(attrs={'class': 'form-control', 'name': 'nomor_kk'}),
            'nik' : forms.TextInput(attrs={'class': 'form-control', 'name': 'nik'}),
            'alamat' : forms.Textarea(attrs={'rows':'5', 'class': 'form-control', 'name': 'alamat'}),

            'rt' : forms.NumberInput(attrs={'class': 'form-control', 'name': 'rt'}),
            'rw' : forms.NumberInput(attrs={'class': 'form-control', 'name': 'rw'}),
            'kode_pos' : forms.TextInput(attrs={'class': 'form-control', 'name': 'kode_pos'}),

            'nomor_hp' : forms.TextInput(attrs={'class': 'form-control', 'name': 'nomor_hp'}), # untuk dihubungi dalam melakukan konfirmasi

            # schedule
            'schedule_time' : forms.Select(attrs={'class': 'form-control', 'name': 'schedule_time'}),
